 ...
 * Global variables
 */
int serverA_sockfd_UDP;
struct sockaddr_in serverA_addr, main_server_addr; // server A's address info and connector's (main server) address info
socklen_t sin_size_main_server;

unordered_map<string, int> transfer_amount;  // will hold the total transferred amount of each person who has transfered so far in the network
unordered_map<string, int> receive_amount;   // will hold the total received amount of each person who has received so far in the network
...
/**
 * Create server A UDP socket, connect to main server, and bind
 */
void serverA_socket_UDP(){

   if((serverA_sockfd_UDP = socket(PF_INET, SOCK_DGRAM, 0)) == -1){
      perror("ERROR: Server A failed to create socket for main server");
      exit(1);
   }
   ...
   // bind to associate socket with port number and IP address of server A side
   // need :: (global scope resolution operator) to use bind for sockets from socket.h
   if(::bind(serverA_sockfd_UDP, (struct sockaddr *)&serverA_addr, sizeof(struct sockaddr)) == -1){
      perror("ERROR: Server A could not bind UDP socket");
      exit(1);
   }

   cout << "The ServerA is up and running using UDP on port " << A_SERVER_PORT_UDP << "." << endl;
}
...
/**
 * Read in block1.txt and create reference for when balance is needed
 */
...
/**
 * Calculates the balance from the transaction file that Server A is assigned to
 */
...
int main(int argc, char* argv[]){
   ...
   serverA_socket_UDP();
   create_transfer_receive_map("block1.txt");
   ...
   while(1){
      ...
      if((temp_vect.size()==1) && (check_monitor_case != "TXLIST")){   // BALANCE CASE
         ...
         if(sendto(serverA_sockfd_UDP, send_back_main_server, sizeof(send_back_main_server), 0, (struct sockaddr *)&main_server_addr, sizeof(main_server_addr)) == -1){
         ...
         // (if transferer is in transfer map OR in receiver map) AND (if receiver is in transfer map OR in receiver map) ==> BOTH users are in the network A
         ...
         // (if ONLY transferrer is in transfer map OR receiver map) OR (if ONLY receiver is in the transfer map OR receiver map) ==> TRANSFERRER or RECEIVER is in network A
         ...
         // neither of them are in network A
         else{
            int transferrer_balance = 0;
            int highest_serial_num = ordered_transactions.rbegin()->first;
         ...
         // to the main server: return balance of transferrer, highest serial number, AND which one or both exists in network A (transferrer, receiver, OR Both)
         if(sendto(serverA_sockfd_UDP, send_back_main_server, sizeof(send_back_main_server), 0, (struct sockaddr *)&main_server_addr, sizeof(main_server_addr)) == -1){
            perror("ERROR: Server A failed to send data to the main server");
            exit(1);
         }
      ...
      For monitor
         - ways to transfer data from backend servers to main server, and thus to client
         - Send in the form of c strings, is there a way to send via data structure? Any suggestions on how to send large data via sockets?
         
         - sending it in c-string
            - sending the transactions, big strings, delimeter
            - buffer, 1024 characters, bigger string in size of 1024
            - while loop sending to the sending server and a while loop to the receiving server
      */
