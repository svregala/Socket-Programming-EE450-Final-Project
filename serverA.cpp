   ...
   while(1){
      ...
      if((temp_vect.size()==1) && (check_monitor_case != "TXLIST")){   // BALANCE CASE
         ...
         if(string(input_from_main_server)!="ERROR"){
            ...
            // if randomizer chose server A to store the transaction file
            if(server_code == "A_STORE"){
               ...
               // Store into text file block1.txt (MUST BE ENCODED APPROPRIATELY)
               ofstream file_out;
               ifstream file_in;
               file_in.open("block1.txt");
               file_out.open("block1.txt", ios::app);  // append to file
               if(file_in.is_open()){
            ...
            int balance_from_A = calculate_balance(sender);
            string temp_balance = to_string(balance_from_A);
