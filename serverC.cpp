...
struct sockaddr_in serverC_addr, main_server_addr; // server B's address info and connector's (main server) address info
...
/**
 * Create server C UDP socket, connect to main server, and bind
 */
void serverC_socket_UDP(){

   if((serverC_sockfd_UDP = socket(PF_INET, SOCK_DGRAM, 0)) == -1){
      perror("ERROR: Server C failed to create socket for main server");
      exit(1);
   }
   ...
   // bind to associate socket with port number and IP address of server C side
   // need :: (global scope resolution operator) to use bind for sockets from socket.h
   if(::bind(serverC_sockfd_UDP, (struct sockaddr *)&serverC_addr, sizeof(struct sockaddr)) == -1){
      perror("ERROR: Server C could not bind UDP socket");
      exit(1);
   }

   cout << "The ServerC is up and running using UDP on port " << C_SERVER_PORT_UDP << "." << endl;
}
...
/**
 * Read in block3.txt and create reference for when balance is needed
 */
...
int main(int argc, char* argv[]){
   ...
   serverC_socket_UDP();
   create_transfer_receive_map("block3.txt");

   while(1){

      // receive data from main server
      sin_size_main_server = sizeof(main_server_addr);
      if(recvfrom(serverC_sockfd_UDP, input_from_main_server, sizeof(input_from_main_server), 0, (struct sockaddr *)&main_server_addr, &sin_size_main_server) == -1){
         perror("ERROR: Server C failed to receive data from the main server");
         exit(1);
      }
      ...
      if((temp_vect.size()==1) && (check_monitor_case != "TXLIST")){   // BALANCE CASE
         ...
         if(sendto(serverC_sockfd_UDP, send_back_main_server, sizeof(send_back_main_server), 0, (struct sockaddr *)&main_server_addr, sizeof(main_server_addr)) == -1){
            perror("ERROR: Server C failed to send data to the main server");
            exit(1);
         }
         ...
         // (if transferer is in transfer map OR in receiver map) AND (if receiver is in transfer map OR in receiver map) ==> BOTH users are in the network C
         ...
         // (if ONLY transferrer is in transfer map OR receiver map) OR (if ONLY receiver is in the transfer map OR receiver map) ==> TRANSFERRER or RECEIVER is in network C
         ...
         // neither of them are in network C
         else{
            int transferrer_balance = 0;
            int highest_serial_num = ordered_transactions.rbegin()->first;
         ...
         if(string(input_from_main_server)!="ERROR"){
            ...
            // if randomizer chose server C to store the transaction file
            if(server_code == "C_STORE"){
               ...
               // Store into text file block3.txt (MUST BE ENCODED APPROPRIATELY)
               ofstream file_out;
               ifstream file_in;
               file_in.open("block3.txt");
               file_out.open("block3.txt", ios::app);  // append to file
               if(file_in.is_open()){
            ...
            int balance_from_C = calculate_balance(sender);
            string temp_balance = to_string(balance_from_C);
